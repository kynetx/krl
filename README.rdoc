...
NOTE: Windows users will need to install a fixed version of the launchy gem in order to have their browser automatically open when a bookmarklet is generated or when krl-connect is run.  You can download the gem from http://github.com/downloads/kynetx/krl/launchy-0.3.5.gem 
Once downloaded, run the following command:
  gem install launchy-0.3.5.gem 
...
info                Displays general information about an application. If run from the 
                    application directoy, no arguments are needed. Otherwise an app id 
                    must be specified. (Note: you must have permission to view the app.)
                    Example (from app dir): krl info
                    or (from anywhere): krl info a1x2
...
update              Overwrites your current .krl file with the specified version.
                    ...
                    Example: krl update
                    or: krl update 10
commit              Uploads your .krl file as the latest version. 
                    Example: krl commit
                    You can also specify a comment for the version you are committing.
                    Example: krl commit "This will be production"
...
generate            Generates an endpoint and places it in the endpoints directory.
                    Name, author, and description are optional and default to the user and app settings.
...
check               Performs a syntax check of your .krl file.
                    The location of the parser can also be added
                    as an optional argument.
                    Example: krl check
                    or: krl check http://0.0.0.0/manage/parse/ruleset
